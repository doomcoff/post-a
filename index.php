...
echo '<div class="spoiler"><div class="spoiler-btn"><h3>'.$y.' <i class="fa fa-angle-down" aria-hidden="true"></i></h3></div>';
    echo '<div class="spoiler-body collapse">';
             $query= ('select distinct DATE_FORMAT(dates, "%Y") y, DATE_FORMAT(dates, "%m") m from `envelope` where DATE_FORMAT(dates, "%Y")="'.$y.'"');
                 $result = mysql_query($query);
                      while( $row = mysql_fetch_array($result)) {
echo '<li class="list-group-item"><a href="select_month.php?m='.$row['y'].'-'.$row['m'].'">'.$monthe_r[$row['m']].'</a></li>';
     }
echo '</div>';
   }
?>
